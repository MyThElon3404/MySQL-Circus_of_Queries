-- QUESTION : 1
-- 1. Find the average total compensation based on employee titles and gender. Total compensation is calculated by adding both the salary and bonus of each employee. However, not every employee receives a bonus so disregard employees without bonuses in your calculation. Employee can receive more than one bonus.
...
-- Step 1: Join Tables to Calculate Total Compensation

SELECT e.id, e.employee_title, e.sex,
    e.salary + SUM(b.bonus) AS total_compensation
FROM sf_employee e
JOIN sf_bonus b 
	ON e.id = b.worker_ref_id
GROUP BY e.id, e.employee_title, e.sex, e.salary;

-- Step 2: Calculate the Average Total Compensation by Title and Gender

SELECT e.employee_title, e.sex,
    AVG(e.salary + SUM(b.bonus)) AS avg_total_compensation
FROM sf_employee e
JOIN sf_bonus b 
	ON e.id = b.worker_ref_id
