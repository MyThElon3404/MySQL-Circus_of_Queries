-- QUESTION : 1
-- 1. Write a query that identifies cities with higher than average home prices when compared to the national average. 
-- Output the city names.
DROP TABLE IF EXISTS zillow_transactions;
...
WITH national_avg AS (
    SELECT AVG(mkt_price) AS national_avg_price
    FROM zillow_transactions
),
city_avg AS (
    SELECT city, AVG(mkt_price) AS city_avg_price
    FROM zillow_transactions
    GROUP BY city
)
SELECT city
FROM city_avg, national_avg
WHERE city_avg.city_avg_price > national_avg.national_avg_price;
-- ==================================================================================================================================

-- QUESTION : 2
-- 2. You are given a table containing assignment scores of students in a class. 
-- Write a query that identifies the largest difference in total score  of all assignments.
-- Output just the difference in total score (sum of all 3 assignments) between a student with the highest score 
-- and a student with the lowest score.
...
INSERT INTO box_scores (id, student, assignment1, assignment2, assignment3) 
VALUES 
(1, 'Karen Austin', 90, 98, 80),(2, 'Matthew Ward', 85, 90, 70),
(3, 'John Miranda', 87, 75, 90), (4, 'Scott Taylor', 60, 55, 70),
(5, 'Tom Saunders', 80, 88, 90), (6, 'Natalie Palmer', 55, 70, 65),
(7, 'James Burgess', 72, 70, 75), (8, 'Tracie Lopez', 89, 85, 70),
(9, 'Donna Jimenez', 91, 90, 98), (10, 'Rachael Smith', 76, 66, 80);
-- SOLUTION :------------------------------------------------------------------------------------------------------------------------
with student_total_score as (
	select id, student,
...
select 
	(max_score - min_score) as largest_diff_max_min
from min_max_score;
-- ==================================================================================================================================
