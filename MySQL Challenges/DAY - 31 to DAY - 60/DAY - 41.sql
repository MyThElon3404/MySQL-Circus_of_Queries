-- QUESTION : 1
-- 1. Calculate the average net earnings per order grouped by weekday (in text format, e.g., Monday) 
-- and hour from customer_placed_order_datetime. 
...
INSERT INTO doordash_delivery 
	(consumer_id, customer_placed_order_datetime, delivered_to_consumer_datetime, delivery_region, discount_amount, 
	driver_at_restaurant_datetime, driver_id, is_asap, is_new, order_total, placed_order_with_restaurant_datetime, 
	refunded_amount, restaurant_id, tip_amount)
VALUES 
...
SELECT DATENAME(weekday, customer_placed_order_datetime) AS day_of_week,
    DATEPART(hour, customer_placed_order_datetime) AS order_hour,
    ROUND(AVG(order_total + tip_amount - discount_amount - refunded_amount), 2) AS avg_earning
FROM doordash_delivery
GROUP BY DATENAME(weekday, customer_placed_order_datetime),
    DATEPART(hour, customer_placed_order_datetime)
ORDER BY 
    CASE DATENAME(weekday, customer_placed_order_datetime)
        WHEN 'Monday' THEN 1
        WHEN 'Tuesday' THEN 2
        ...
        WHEN 'Sunday' THEN 7
        ELSE 8 -- Optional, for invalid values
    END, 
    order_hour;
